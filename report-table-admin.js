...
(function reportTableAdminModule(global){
  ...
  function toolbarHtml(config){
    ...
    return '<div class="rta-toolbar"><div><b>'+escHtml(LABELS[activeType])+'标准表格库</b><span>'+stats.templates+'套逻辑表 · '+stats.physicalTables+'张源Word物理表 · 当前 v'+Number(config.version||1)+'</span></div><div class="rta-actions"><select id="rtaType">'+Object.keys(LABELS).map(type=>'<option value="'+type+'" '+(type===activeType?'selected':'')+'>'+escHtml(LABELS[type])+'</option>').join('')+'</select>'
  ...
  function render(){
    ...
    lazyBind();document.getElementById("rtaExport").onclick=exportWord;document.getElementById("rtaType").onchange=async event=>{activeType=event.target.value;editing=false;await ensureLoaded(false);render();};
