...
(function reportTableAdminModule(global){
  "use strict";
  const TYPE="rent",escHtml=value=>typeof esc==="function"?esc(value):String(value||"").replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[char]));
  let mountedRoot=null,workingSet=null,editing=false,busy=false;
  const clone=value=>JSON.parse(JSON.stringify(value));

  async function ensureLoaded(force){
    if(!global.ReportTableTemplates)throw new Error("表格模板模块未加载");
    const set=await global.ReportTableTemplates.load(TYPE,!!force);workingSet=clone(set);return set;
  }
  ...
  function cardBody(index){
    ...
    return '<div class="rta-title-edit"><label>表格名称</label><input data-rta-title value="'+escHtml(template.title)+'" '+(editing?'':'readonly')+'></div>'
  ...
  function lazyBind(){
    mountedRoot.querySelectorAll("details[data-rta-index]").forEach(card=>card.ontoggle=()=>{
      if(!card.open)return;const body=card.querySelector(".rta-lazy");if(body&&!body.dataset.loaded){body.innerHTML=cardBody(card.dataset.rtaIndex);body.dataset.loaded="1";}
    });
  }
  function toolbarHtml(config){
    return '<div class="rta-toolbar"><div><b>出租类标准表格库</b><span>33套逻辑表 · 70张原报告物理表 · 当前 v'+Number(config.version||1)+'</span></div><div class="rta-actions">'
      +'<button class="btn sm ghost" id="rtaExport">📄 导出表格Word版</button>'
      +(editing?'<button class="btn sm" id="rtaPublish">保存并同步到前台</button><button class="btn sm ghost" id="rtaCancel">取消修改</button>':'<button class="btn sm" id="rtaEdit">🔓 修改表格模板</button>')+'</div></div>';
  }
  ...
  function render(){
    ...
    lazyBind();document.getElementById("rtaExport").onclick=exportWord;
    if(editing){document.getElementById("rtaPublish").onclick=publish;document.getElementById("rtaCancel").onclick=cancel;}else document.getElementById("rtaEdit").onclick=beginEdit;
  }
  async function beginEdit(){
    const proceed=()=>{editing=true;workingSet=clone(global.ReportTableTemplates.current(TYPE));render();};
  ...
  async function publish(){
    if(busy)return;readAll();const overrides=global.ReportTableTemplates.buildOverrides(workingSet,global.ReportTableTemplates.baseline(TYPE));
    const changed=Object.keys(overrides.templates||{}).length;if(!confirm("确认发布表格模板新版本？共涉及 "+changed+" 张逻辑表，发布后前台立即使用。"))return;
    const button=document.getElementById("rtaPublish");busy=true;button.disabled=true;button.textContent="正在保存…";
    try{
      const response=await fetch("/api/reporttables",{method:"POST",headers:authHeaders(),body:JSON.stringify({action:"publish",projectType:TYPE,overrides})}),result=await response.json();
      if(!response.ok||!result.ok)throw new Error(result.error||"表格模板发布失败");editing=false;await ensureLoaded(true);render();if(typeof msg==="function")msg("出租类表格模板已发布为 v"+result.config.version,"ok");
  ...
  async function exportWord(){
    ...
    try{
      ...
      a.href=url;a.download="出租类可研标准表格模板库-v"+Number(global.ReportTableTemplates.config(TYPE).version||1)+".docx";document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url);
    }catch(error){alert("表格Word导出失败："+error.message);}finally{button.disabled=false;button.textContent="📄 导出表格Word版";}
  }
  async function mount(root){
    mountedRoot=root;if(!root)return;root.innerHTML='<div class="empty">加载出租类标准表格库…</div>';
  ...
  if(typeof module==="object"&&module.exports)module.exports={chapterGroups};
