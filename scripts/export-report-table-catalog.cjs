...
const root=path.resolve(__dirname,".."),set=JSON.parse(fs.readFileSync(path.join(root,"data","report-table-templates-rent-v1.json"),"utf8"));
const sections=list=>list.map(template=>({title:template.title,blocks:[{type:"templateTable",template}]}));
const payload={
  project:{name:"出租类可研标准表格模板库 v1",owner:"公司可研生成逻辑库",industry:"保障性住房可行性研究"},signed:true,docNo:"",
  chapters:[
...
const output=process.argv[2]?path.resolve(process.argv[2]):path.join(root,"outputs","出租类可研标准表格模板库-v1.docx");
