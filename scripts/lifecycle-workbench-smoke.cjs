...
(async()=>{
 ...
 const errors=[],requests=[],writes=[],state={role:'OWNER',failOps:false,failBrain:false,failIntelligence:false,emptyRequirements:false,delayBrain:100,delayFiles:0,failList:false,stage:'feasibility',version:1};
 ...
 const select=async id=>{await page.locator('[data-pm-select="'+id+'"]').click();await page.locator('.pm-stage-workspace').waitFor();};
 ...
 const tool=async view=>{await page.locator('#pmSidebar [data-pw-view="'+view+'"]').click();await page.waitForFunction(v=>location.hash.endsWith('/'+v),view);};
 try{
  await page.goto(fixture);await page.locator('[data-pm-select]').first().waitFor();await select('project-test-a');
  await page.locator('[data-pm-work-stage]').waitFor();await page.getByText('project-test-a-可研与尽调',{exact:true}).waitFor();
  assert.equal(await page.locator('#pmSidebar [data-pw-view]').count(),12);
  ...
  await tool('actions');await page.locator('#pmMeetingText').fill('[系统测试]未提交会议草稿');await stage('decision');await tool('actions');assert.equal(await page.locator('#pmMeetingText').inputValue(),'[系统测试]未提交会议草稿');
  await page.evaluate(()=>window.rejectSave=true);await stageAttempt('initiation');assert.ok(page.url().endsWith('/actions'));assert.equal(await page.locator('#pmMeetingText').inputValue(),'[系统测试]未提交会议草稿');await page.evaluate(()=>window.rejectSave=false);
  await stage('decision');await page.locator('[data-pm-library]').click();await select('project-test-b');assert.ok(page.url().endsWith('/feasibility'));
  await stage('initiation');await page.locator('[data-pm-library]').click();await select('project-test-a');assert.ok(page.url().endsWith('/decision'));
  await page.locator('#ppClose').click();await page.evaluate(()=>{window.testUser='owner-b';return enter();});await select('project-test-a');assert.ok(page.url().endsWith('/feasibility'),'another user does not inherit navigation');
  for(const view of ['overview','stages','facts','data','files','spatial','decisions','actions','scenarios','versions','members','acceptance']){await tool(view);await page.locator('#pmDetail .pm-page-heading').waitFor();}
  ...
  state.emptyRequirements=true;await page.reload();await page.getByText('后台清单当前为零项，不代表资料自动通过专业复核。',{exact:true}).waitFor();state.emptyRequirements=false;await stage('decision');await page.reload();await page.locator('.pm-stage-workspace').waitFor();await page.waitForFunction(()=>document.querySelectorAll('.pm-stage-workspace .pm-workspace-loading').length===0);
  ...
  state.role='VIEWER';await page.reload();await page.locator('.pm-stage-workspace').waitFor();assert.equal(await page.locator('[data-pm-work-stage]').count(),0);assert.equal(await page.locator('[data-pm-ai]').count(),0);for(const s of stages)await stage(s.key);await tool('overview');assert.equal(await page.locator('[data-pm-open]').count(),0);await tool('actions');assert.equal(await page.locator('[data-pm-extract]').count(),0);assert.equal(await page.locator('#pmMeetingText').isDisabled(),true);
  await tool('facts');await page.locator('[data-requirement-status="missing"]').waitFor();assert.equal(await page.locator('.pm-requirement-row').count(),5);
  await page.locator('#ppClose').click();state.failList=true;await page.locator('#reopen').click();await page.locator('#pmRetryList').waitFor();state.failList=false;await page.locator('#pmRetryList').click();await page.locator('[data-pm-select]').first().waitFor();
  assert.deepEqual(errors,[]);console.log(JSON.stringify({ok:true,requests:requests.length,navigationWrites:0,explicitStageWrites:writes.length,consoleErrors:errors,states:['8 stages','12 tools','requirements/missing/assumption/conflict/NA','requirements-fallback/error/empty','refresh','back-forward','draft-preserved','save-rejection','user/project-isolation','partial-failure/retry','component-failure','late-response','OWNER-versioned-stage/undo','VIEWER','375/800/1440px','list-failure/retry']}));
