...
/* ===== 启动自检：这份代码同时有多方在改，已经出现过两次"某个跨文件共享的函数被覆盖掉，
   界面点了没反应/报'xxx is not defined'"——用户点到那一步才发现，摸不着头脑。
   这里在页面刚加载完就检查一遍这些函数是否都还在，缺了就在页面顶部挂一条显眼的红条，
   把缺了什么直接列出来，不用等点到那个功能才暴露问题。
   （只检查"函数是否存在"，不校验实现是否正确——那是回归测试该管的事，这里只防"整个没了"。）*/
const SELF_CHECK_FUNCTIONS = [
  "runCalcEngine", "runWorkerPool", "loadDomain", "generateSection", "saveDraft", "getOutlines",
...
function selfCheckCriticalFunctions(){
  const missing = SELF_CHECK_FUNCTIONS.filter(name => typeof window[name] !== "function");
  Object.entries(SELF_CHECK_ENGINES).forEach(([label, get])=>{
    const obj = get();
    if(!obj || typeof obj.calc !== "function") missing.push(label);
  });
  if(!missing.length) return;
  console.error("[启动自检失败] 以下函数/对象缺失：", missing);
  const bar = document.createElement("div");
  bar.id = "selfCheckBar";
  bar.style.cssText = "position:fixed; top:0; left:0; right:0; z-index:99999; background:#C24A42; color:#fff; "
    + "font-size:12.5px; line-height:1.6; padding:10px 18px; text-align:center; box-shadow:0 2px 8px rgba(0,0,0,.2);";
  bar.innerHTML = "⚠ 代码自检失败，以下功能可能无法使用：<b>" + missing.join("、") + "</b>"
    + "　——　代码可能被并发编辑覆盖，请检查对应文件后刷新页面。";
