...
(function webResearchToolsModule(global){
  ...
  function buildBatchTargets(items){
    ...
    const rank={A:4,B:3,C:2,D:1},sectionTargets=[...groups.values()].map((group,index)=>{
      const decisions=group.requirements.map(x=>x.decision).filter(Boolean),reuseKeys=[...new Set(decisions.map(x=>x.reuseKey).filter(Boolean))],fields=[],fieldKeys=new Set(),queryTerms=[];
      group.requirements.forEach(req=>{(req.fields||[]).forEach(field=>{const key=String(field.key||field.label||"");if(key&&!fieldKeys.has(key)){fieldKeys.add(key);fields.push(field);}});(req.queryTerms||[]).forEach(term=>{if(term&&!queryTerms.includes(term))queryTerms.push(term);});});
      const qualities=group.requirements.map(x=>x.quality||{}),minAuthority=qualities.map(x=>x.minAuthority||"D").sort((a,b)=>(rank[b]||0)-(rank[a]||0))[0]||"A",maxAge=group.requirements.map(x=>Number(x.timeScope?.maxAgeMonths)).filter(Number.isFinite),primary=group.requirements.slice().sort((a,b)=>Number(b.decision?.priority||0)-Number(a.decision?.priority||0))[0]||{};
      const target={...group,index,bindings:[{chapter:group.chapter,section:group.section,logicIds:group.logicIds.slice()}],requirement:group.requirements.map(x=>x.evidenceGoal).filter(Boolean).join("；")||group.titles.join("；")||"本节公开依据",query:group.requirements.map(x=>x.query).filter(Boolean).join("；").slice(0,180),reuseKey:reuseKeys.length===1?reuseKeys[0]:"",decisionPriority:Math.max(0,...decisions.map(x=>Number(x.priority)||0)),decisionReasons:[...new Set(decisions.map(x=>x.reason).filter(Boolean))],budget:{maxQueries:1,maxResults:5,maxOutputTokens:900,maxSourcesAccepted:2},quality:{minScore:Math.max(80,...qualities.map(x=>Number(x.minScore)||0)),minAuthority,requireCrossCheck:qualities.some(x=>x.requireCrossCheck)}};
      target.requirementSchema={...primary,requirementId:"batch:"+(target.reuseKey||group.logicIds.join("+")),ruleId:group.logicIds[0]||"",fields:fields.slice(0,20),queryTerms:queryTerms.slice(0,12),query:target.query,budget:target.budget,quality:target.quality,timeScope:{...(primary.timeScope||{}),maxAgeMonths:maxAge.length?Math.min(...maxAge):primary.timeScope?.maxAgeMonths},decision:{...(primary.decision||{}),priority:target.decisionPriority,reuseKey:target.reuseKey}};return target;
    });
    const consolidated=[];
    for(const target of sectionTargets){const existing=target.reuseKey&&consolidated.find(x=>x.reuseKey===target.reuseKey);if(!existing){consolidated.push(target);continue;}existing.logicIds.push(...target.logicIds.filter(id=>!existing.logicIds.includes(id)));existing.requiredSources.push(...target.requiredSources.filter(x=>!existing.requiredSources.includes(x)));existing.requirements.push(...target.requirements.filter(req=>!existing.requirements.some(x=>x.requirementId===req.requirementId)));existing.sourceNos.push(...target.sourceNos.filter(x=>!existing.sourceNos.includes(x)));existing.titles.push(...target.titles.filter(x=>!existing.titles.includes(x)));existing.bindings.push(...target.bindings);existing.decisionPriority=Math.max(existing.decisionPriority,target.decisionPriority);existing.requirement=[existing.requirement,target.requirement].filter((x,i,a)=>x&&a.indexOf(x)===i).join("；");}
    consolidated.sort((a,b)=>b.decisionPriority-a.decisionPriority||Math.min(...a.sourceNos)-Math.min(...b.sourceNos));return consolidated.map((target,index)=>({...target,index}));
  }
  ...
  function batchDecisionLabel(target){
  ...
  function batchSignature(targets){return targets.map(target=>[target.reuseKey,target.chapter,target.section,target.query,...target.logicIds,...(target.requirements||[]).map(x=>x.requirementId+"@"+(x.refinementVersion||0))].join("|")).join("\n");}
  ...
  async function searchBatchTarget(job,target){
    ...
    const data=await api(Object.assign({},ctx,{action:"searchSection",requirementSchema:target.requirementSchema||target.requirements&&target.requirements[0]||null,budget:target.budget,maxQueries:1,maxResults:Math.min(job.maxResults,5)}));
