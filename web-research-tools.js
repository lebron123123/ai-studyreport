...
(function webResearchToolsModule(global){
  ...
  async function depositHighValue(job,levels,options){
    options=options||{};const entries=highValueEntries(job,levels),results=await runLimited(entries,Math.min(4,Number(options.concurrency)||3),async entry=>{try{const data=await submitKnowledgeReview(entry.row,entry.target,{fetchFullText:options.fetchFullText===true});entry.occurrences.forEach(x=>{x.row.knowledgeDeposit={status:data.existing?"existing":"submitted",id:data.id||"",auto:!!options.auto,at:Date.now()};});return {ok:true,existing:!!data.existing};}catch(error){entry.occurrences.forEach(x=>{x.row.knowledgeDeposit={status:"failed",error:error.message||String(error),auto:!!options.auto,at:Date.now()};});return {ok:false,error:error.message||String(error)};}});if(options.persist!==false)persistBatchJob(job);return {total:entries.length,submitted:results.filter(x=>x&&x.ok&&!x.existing).length,existing:results.filter(x=>x&&x.ok&&x.existing).length,failed:results.filter(x=>x&&!x.ok).length};
  }
