...
async function handle(context,post){
  ...
  try{return json({ok:true,result:post?await deliveryAction(env,user.userId,await context.request.json()):await listDeliveries(env,user.userId,url.searchParams.get('projectId'),url.searchParams.get('id'),url.searchParams.get('riskReportId'))});}catch(e){return json({ok:false,error:e.message},409);}
}
