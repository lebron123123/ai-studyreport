...
export function scoreReportCase(sample,text){
  const checks=[...sample.required.map(value=>({kind:'required',value,passed:text.includes(value)})),...sample.forbidden.map(value=>({kind:'forbidden',value,passed:!text.includes(value)}))];
  const passed=!!text.trim()&&checks.length>0&&checks.every(x=>x.passed);
  return {passed,score:checks.length?Math.round(checks.filter(x=>x.passed).length/checks.length*100):0,checks,method:'deterministic-constraints-v1',semanticAccuracyVerified:false};
}
...
export async function registerReportCase(env,userId,input){
  const sample={projectId:String(input.projectId||''),scenario:String(input.scenario||''),datasetRole:input.datasetRole,input:String(input.input||''),required:input.required,forbidden:input.forbidden||[],approvalNote:String(input.approvalNote||'')};
  if(!sample.projectId||!sample.scenario||!['training','holdout'].includes(sample.datasetRole)||!sample.input||!sample.approvalNote||!Array.isArray(sample.required)||!Array.isArray(sample.forbidden)||sample.required.length+sample.forbidden.length===0||[...sample.required,...sample.forbidden].some(x=>typeof x!=='string'||!x.trim()))throw new Error('需要确认项目、场景、training/holdout、真实输入、检查条件和负责人批准依据');
  if(JSON.stringify(sample).length>25000)throw new Error('样本过大，请拆分为可独立核验的小节');
...
async function enqueueReportEvaluation(env,userId,row,caseId){
  ...
  const job=await enqueueAgentJob(env,userId,run.id,{kind:'llm_task',maxAttempts:1,payload:{projectId:row.project_id,system:'依据项目材料执行以下写作规则。仅输出报告正文，不要输出说明。\n'+binding.rule,query:sample.input,maxTokens:4000}});
