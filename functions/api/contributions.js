...
function normalizeRegionScope(value,meta={}){
  const raw=clean(value||meta.region||"",80).replace(/\s+/g,"");
  if(raw==="全国"||meta.regionLevel==="national")return {region:"全国",regionLevel:"national",regionPath:["全国"]};
  const city=raw.match(/([\u4e00-\u9fa5]{2,10}市)/),local=city?raw.slice(city[1].length):raw,street=local.match(/([\u4e00-\u9fa5]{2,8}区)([\u4e00-\u9fa5]{2,12}街道)/),district=local.match(/([\u4e00-\u9fa5]{2,8}区)/);
  ...
  return {region:raw||"深圳市",regionLevel:meta.regionLevel||"city",regionPath:Array.isArray(meta.regionPath)&&meta.regionPath.length?meta.regionPath:[raw||"深圳市"]};
}
function out(row){if(!row)return null;const meta=safeJson(row.meta,{}),scope=normalizeRegionScope(row.region,meta);return {...row,region:scope.region,meta:{...meta,...scope},created_at:Number(row.created_at||0),reviewed_at:Number(row.reviewed_at||0)};}
...
async function routeApproved(env,row,user){
  const now=Date.now(), meta=safeJson(row.meta,{}),scope=normalizeRegionScope(row.region,meta);row={...row,region:scope.region};Object.assign(meta,scope);
  if(row.kind==="wiki"){
  ...
  if(row.kind==="report_logic"){
    const projectType=clean(row.project_type||meta.projectType||"rent",30),current=await env.DB.prepare("SELECT * FROM report_logic_sets WHERE project_type=? AND status='published' ORDER BY version DESC LIMIT 1").bind(projectType).first();
    if(!current)throw new Error("当前项目类型尚无已发布生成逻辑");
    const revision=meta.revision&&typeof meta.revision==="object"?meta.revision:safeJson(row.content,{}),evaluation=evaluateRuleRevisionData(safeJson(current.data,{}),{projectType,baseRuleId:meta.baseRuleId,businessScenario:meta.businessScenario,revision});
    if(!evaluation.recommended)throw new Error("候选逻辑未通过自动评测："+[...evaluation.blockers,...evaluation.warnings].join("；"));
    const data=mergeRuleRevisionData(safeJson(current.data,{}),{projectType,baseRuleId:meta.baseRuleId,businessScenario:meta.businessScenario,revision},user.username||String(user.userId),now);
    const latest=await env.DB.prepare("SELECT version FROM report_logic_sets WHERE project_type=? ORDER BY version DESC LIMIT 1").bind(projectType).first(),version=Number(latest?.version||0)+1,id=`report-logic-${projectType}-v${version}-${now.toString(36)}`;
    ...
    return {module:"可研逐小节生成逻辑 v"+version,ref:meta.baseRuleId||id};
  }
...
export async function onRequestPost(context){
  ...
  if(action==="submit"){
    ...
    const id=makeId(),now=Date.now(),meta=JSON.stringify({...itemMeta,...scope});
