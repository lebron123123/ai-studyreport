...
export async function authorizeAgentAction(env,principal,req={}){
  ...
  const access=await resolveProjectAccess(env,principal.userId,projectId);
  if(!access)return {ok:false,reason:"无该项目访问授权或成员已被移除"};
  // Project roles do not grant formal report approval authority.
  if(!['read','write'].includes(action))return {ok:false,reason:"本次项目角色不包含正式审批授权"};
  if(action==='write'&&!access.permissions.edit)return {ok:false,reason:"查看者不能执行项目生成或修改任务"};
  if(access.role==='OWNER')return {ok:true,scope:'owner',role:access.role};
  ...
  if(!grant)return {ok:true,scope:'member',role:access.role};
