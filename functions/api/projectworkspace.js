...
const TABLES=[
  ...
  "CREATE TABLE IF NOT EXISTS rag_source_objects (content_hash TEXT PRIMARY KEY,storage_key TEXT NOT NULL UNIQUE,file_name TEXT NOT NULL DEFAULT '',mime_type TEXT NOT NULL DEFAULT 'application/octet-stream',size_bytes BIGINT NOT NULL DEFAULT 0,created_by INTEGER,created_at BIGINT NOT NULL,verified_at BIGINT NOT NULL DEFAULT 0)",
...
async function registerFileRecord(env,a,user,projectId,file,now){
  ...
  const old=await one(env,"SELECT id,version FROM project_files WHERE project_id=? AND LOWER(file_name)=LOWER(?) AND is_current=1 ORDER BY version DESC",projectId,name),version=Number(old&&old.version||0)+1,fileId=id("pfile");
  if(old)await env.DB.prepare("UPDATE project_files SET is_current=0,status='superseded',updated_at=? WHERE project_id=? AND LOWER(file_name)=LOWER(?) AND is_current=1").bind(now,projectId,name).run();
  await env.DB.prepare("INSERT INTO project_files(id,project_id,owner_user_id,file_name,file_type,category,storage_ref,fingerprint,version,status,parse_status,is_current,parent_file_id,size_bytes,meta_json,created_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)").bind(fileId,projectId,a.ownerUserId,name,f.fileType,f.category,f.storageRef,f.fingerprint,version,"registered","pending",1,old&&old.id||"",f.sizeBytes,JSON.stringify(f.meta||{}),clean(user.username||user.userId,80),now,now).run();
  await event(env,user,projectId,"project.file.registered",{fileId,name,version,parentFileId:old&&old.id||"",storageRef:f.storageRef||"",fingerprint:f.fingerprint||""});
...
export async function onRequestPost(contextArg){
  ...
  if(action==="registerFile"){
    if(!a.permissions.edit)return json({ok:false,error:"当前角色无权登记文件"},403);try{return json({ok:true,...await registerFileRecord(env,a,user,projectId,b.file||{},now)});}catch(error){return json({ok:false,error:error.message},400);}
  }
  if(action==="storeProjectOriginal"){
    if(!a.permissions.edit)return json({ok:false,error:"当前角色无权保存项目原件"},403);
    if(!env.RAG_OBJECTS||typeof env.RAG_OBJECTS.put!=="function")return json({ok:true,stored:false,warning:"当前部署没有配置原件对象存储；解析文本仍会保存到项目草稿"});
    const name=clean(b.name||"source.bin",220),mimeType=clean(b.mimeType||"application/octet-stream",120),base64=String(b.dataBase64||"");
    if(!base64)return json({ok:false,error:"原件内容为空"},400);if(base64.length>70*1024*1024)return json({ok:false,error:"单个原件超过约50MB，请拆分后上传"},400);
    try{
      const binary=atob(base64),bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);
      const stored=await env.RAG_OBJECTS.put({bytes,fileName:name,mimeType}),old=await one(env,"SELECT content_hash FROM rag_source_objects WHERE content_hash=?",stored.contentHash);
      ...
      const registered=await registerFileRecord(env,a,user,projectId,{name,fileType:name.includes(".")?name.split(".").pop().toLowerCase():"",category:"project_material",storageRef:stored.storageKey,fingerprint:stored.contentHash,sizeBytes:stored.sizeBytes,meta:{mimeType,objectStored:true,deduplicated:!!stored.deduplicated}},now);
      return json({ok:true,stored:true,object:stored,...registered});
    }catch(error){return json({ok:false,error:"项目原件保存失败："+error.message},500);}
  }
