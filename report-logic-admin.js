...
(function reportLogicAdminModule(global){
  ...
  const TYPE_LABELS={rent:"出租类",gaibao:"非居改保"};
  let activeSet=null, activeType="rent", editing=false;
  ...
  function renderSet(){
    ...
    root.innerHTML='<div class="rla-summary"><div><b>'+rules.length+'</b><span>正式逻辑项</span></div><div><b>'+groups.size+'</b><span>一级章节</span></div><div><b>v'+activeSet.version+'</b><span>当前发布版</span></div><div><b>'+(editing?'编辑中':'前台交互')+'</b><span>'+(editing?'发布后生效':'日常完善入口')+'</span></div></div><div class="msg '+(editing?'warn':'ok')+'"><b>'+(editing?'编辑说明：':'当前口径：')+'</b>'+(editing?'修改不会立即影响用户；点击“校验并发布新版本”后才生效，系统会自动重新编号并保留上一版本。':'以《'+escHtml(activeSet.data?.source?.fileName||TYPE_LABELS[activeType]+'逻辑表')+'》实际 '+rules.length+' 项为准，当前按顺序编号1—'+rules.length+'；出租类与非居改保分类治理，生成与审核共用同一规则ID。')+'</div>'
      ...
      +(activeType==="rent"?'<div id="rlaTablePanel"></div>':'');
    if(activeType==="rent"&&global.ReportTableAdmin)global.ReportTableAdmin.mount(document.getElementById("rlaTablePanel"));
    if(!editing)return;
    document.getElementById("rlaAdd").onclick=()=>{readEditors();const last=activeSet.data.rules.at(-1)||{};activeSet.data.rules.push({id:activeType+"-logic-"+Date.now().toString(36),projectType:activeType,chapter:last.chapter||"第十四章 研究结论与建议",section:"新增小节",subsection:"",pointTitle:"",displayTitle:"新增小节",requiredSources:"",sourceKinds:[],writingLogic:"",importance:"★★一般",outputForm:"文字",generationMode:"ai_writing",missingPolicy:"资料缺失时标注待补，不得虚构",note:"",projectSpecific:false});renderSet();};
  ...
  async function publishChanges(){
    ...
    try{await api({action:"publish",projectType:activeType,data:activeSet.data});editing=false;if(typeof stdEditUnlocked!=="undefined")stdEditUnlocked=false;activeSet=await loadPublished(activeType);if(typeof msg==="function")msg(TYPE_LABELS[activeType]+"可研生成逻辑已发布为 v"+activeSet.version,"ok");renderReportLogicTab();}catch(error){button.disabled=false;button.textContent="校验并发布新版本";alert("发布失败："+error.message);}
  }
