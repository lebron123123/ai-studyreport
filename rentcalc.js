...
function calc(p){
  ...
  const singleReset = p.decorationCost * 0.7;
  const resetPeriod = p.houseType==="公租房"? 20 : 10;
  const resetDict={}; operateArr.forEach(y=>resetDict[y]=0);
  for(let rn=resetPeriod; rn<=maxOpNum; rn+=resetPeriod){
    const end = Math.min(rn+9, maxOpNum);
  ...
  allYears.forEach(y=>{
    ...
    const mgH = p.area*occ*12*1.92*p.manageCoeff/10000;
    const mgP = parkInc*0.4;
    const ins = p.totalBuildArea*0.3/10000;
    const rep = (p.area*rent*occ*m/10000)*0.02;
    const fund = p.area*occ*m*0.25/10000;
    const vac = p.area*(1-occ)*m*3.9/10000;
    const reset = resetDict[y];
    const dep = opIndex[y]<=50? p.totalInvestment*(1-0.2)/50 : 0;
  ...
  allYears.forEach(y=>{
    ...
    const vat = resi*(0.015/1.05) + park*(0.09/1.09);
    const stamp = tot*(0.0005/1.09);
    const city = vat*0.07, edu = vat*0.05;
    let prop=0;
    if(opIndex[y]>3){
      prop = resi*occ*(0.04/1.05) + park*(0.12/1.09) + (p.constructionCost*0.7*0.012/1.09)*(1-occ)*(m/12);
    }
    const land = p.landArea*3/10000;
