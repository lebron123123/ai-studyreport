// Adapted from local GTA_SZ derivative: city-night-sky.ts; original SHA256 473e24cbc0884c2bd6fb58e3c70b2af91a10d4e00104637fba99e507044c2296
const { Engine, RawCubeTexture, ShaderMaterial, Texture, Vector3 } = globalThis.BABYLON;
export const CITY_MOON_DIRECTION = new Vector3(.72, .48, -.5).normalize();
export function createCityNightSky(scene) {
   ...
   void main(){
    ...
    for(let face = 0; face < 6; face++){
        ...
        for(let y = 0; y < size; y++)for(let x = 0; x < size; x++){
            ...
            d.normalize();
            const h = Math.max(0, d.y), horizon = Math.exp(-Math.abs(d.y) * 5);
            const color = d.y < 0 ? [
                .040,
                .037,
                .038
            ] : [
                .043 - .018 * h + .012 * horizon,
                .051 - .017 * h + .006 * horizon,
                .078 - .021 * h
            ];
    ...
    environment.name = 'city-neutral-night-environment';
