...
(function(root){
  ...
  function rgNumber(v){if(v===null||v===undefined||String(v).trim()==='')return null;const n=Number(v);return Number.isFinite(n)?n:null;}
  ...
  function evaluate(sample,candidate){
    ...
    // Legacy client score is diagnostic only. Source-count confidence is not accuracy evidence.
    const trust=0;
    const audit=candidate.preSubmitAudit||{},auditScore=audit.ready===true?Math.max(0,1-(Number(audit.warningCount)||0)*0.02):0;
    ...
    const score=Math.round((coverage*0.35+factAccuracy*0.35+(1-missingRate)*0.2+auditScore*0.1)*100);
    return {schemaVersion:3,diagnosticOnly:true,productionEligible:false,datasetRole:sample.datasetRole==="holdout"?"holdout":"training",sourceProjectId:String(sample.sourceProjectId||""),score:factRows.length?score:null,grade:'客户端诊断 · 非正式评测',metrics:{sectionCoverage:Math.round(coverage*100),factAccuracy:factRows.length?Math.round(factAccuracy*100):null,factualErrorRate:factRows.length?Math.round((1-factAccuracy)*100):null,humanModificationDistance:Math.round(modificationDistance*100),missingRate:Math.round(missingRate*100),trustScore:null,auditScore:Math.round(auditScore*100),unnecessaryWebQueries:unnecessary},factResults,missingSections:matched.filter(x=>!x.actual||!x.actual.text).map(x=>x.expected.title),passed:false,diagnosticPassed:factRows.length>0&&score>=80&&factAccuracy>=0.95&&audit.ready===true};
  }
