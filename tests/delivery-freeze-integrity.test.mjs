...
import {assertExpectedDeliveryHash,verifyFrozenDelivery,verifyDeliverySnapshot,deliverySnapshot} from '../functions/api/_delivery.js';
...
async function fixture(){
 ...
 return {hash,row:{content_hash:hash,snapshot_json:JSON.stringify(snapshot),contract_json:JSON.stringify({expected:[{label:'总投资',value:100,unit:'万元',sourceRef:'confirmed-calculation',version:1}]}),result_json:JSON.stringify({passed:true})}};
}
...
test('新冻结格式封存引用与逻辑，段落ID稳定且元数据变化不随机改变哈希',async()=>{
 ...
 assert.equal(snapshot.schemaVersion,3);assert.equal(section.paragraphs.length,2);assert.deepEqual(section.paragraphs.map(p=>p.id),['c1:s1:p1','c1:s1:p2']);
...
test('原v1哈希逐字兼容，既有冻结内容不补写字段或假造来源',async()=>{
 ...
 assert.deepEqual(await verifyDeliverySnapshot(row),legacy);await assert.rejects(()=>verifyFrozenDelivery(row,hash),/重新检查未通过/);assert.equal(row.snapshot_json,JSON.stringify(legacy));
});
