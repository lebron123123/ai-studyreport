...
function makeDb(){
  const state={project:{id:"project-workspace",name:"企业项目",user_id:1,updated_at:10,data:"{}"},profile:null,members:[],files:[],extractions:[],facts:[],metrics:[],artifacts:[],issues:[],events:[]};
...
async function call(DB,method,body,url="http://test/api/projectworkspace?projectId=project-workspace&view=files",userId=1){const env={DB,SESSION_SECRET:"workspace-test",DEPLOY_MODE:"local"},token=await signToken(env,userId,"tester"),headers={authorization:"Bearer "+token};if(body)headers["content-type"]="application/json";const request=new Request(url,{method,headers,body:body?JSON.stringify(body):undefined}),res=await(method==="GET"?onRequestGet:onRequestPost)({request,env});return {status:res.status,data:await res.json()};}
