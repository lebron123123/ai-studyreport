...
test("四类信息标签能同时表达白箱计算、事实依据、AI判断和待核假设",()=>{
  ...
  assert.equal(p.score,null);assert.equal(p.status,'missing');
});
...
test("无证据AI正文不会被误标成事实，高置信度必须有来源",()=>{
  ...
  assert.equal(p.score,null);assert.equal(p.status,'unverified');
});
...
test("报告可信度汇总准确列出低可信与待补小节",()=>{
  ...
  assert.equal(s.total,2);assert.equal(s.attention.length,2);assert.equal(s.statusCounts.missing,1);assert.equal(s.averageScore,null);
});
